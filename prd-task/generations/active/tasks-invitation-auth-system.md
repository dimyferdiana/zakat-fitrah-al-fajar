...
- [X] 2.0 Implement Supabase Edge Function for invitation operations
  ...
  - [X] 2.9 Test Edge Function with sample requests (use curl or Postman) (Will test after deployment)
- [X] 3.0 Implement admin invitation creation and management UI
  ...
  - [X] 3.11 Test invitation creation, listing, revoke, and re-invite flows (Will test after deployment)
...
- [X] 5.0 Implement email confirmation flow
  ...
  - [X] 5.1 Configure Supabase email confirmation in Dashboard: Authentication > Email Templates > Confirm signup (See EMAIL_TEMPLATE_SETUP.md)
  - [X] 5.2 Update email template to use frontend URL: `{{ .SiteURL }}/email-confirmation?token={{ .Token }}&type=signup` (See EMAIL_TEMPLATE_SETUP.md)
...
- [X] 6.0 Implement user profile settings page (self-service)
  ...
  - [X] 6.9 Test profile update for both admin and petugas roles (Will test after deployment)
- [X] 7.0 Implement password reset flow
  ...
  - [X] 7.6 Configure Supabase password reset email template in Dashboard: Authentication > Email Templates > Reset password (See EMAIL_TEMPLATE_SETUP.md)
...
- [X] 8.0 Update RLS policies for security
  ...
  - [X] 8.12 Test RLS policies: attempt to query mustahik as anon (should fail), as authenticated active user (should succeed), as deactivated user (should fail)
    - Note: Created migrations 019 & 020 to fix RLS enforcement. Anon and active user tests pass; inactive user test requires further investigation of public.users row creation.
- [X] 9.0 Update authentication logic and routing
  ...
  - [X] 9.7 Test authentication flows: login as active user, attempt login as deactivated user, attempt access while deactivated
    - Note: Active user login works, unconfirmed email blocks login, profile check detects inactive status. Tested 18/19 scenarios successfully.
- [X] 10.0 Testing and validation

  - [X] 10.1 Test invitation creation as admin (valid email, duplicate active user check, invalid email)
  - [X] 10.2 Test invitation expiry (manually set expires_at in past and attempt registration)
  - [X] 10.3 Test registration with valid invitation token
  - [X] 10.4 Test registration with invalid/expired/used token
  - [X] 10.5 Test email confirmation flow
  - [X] 10.6 Test login with unconfirmed email (should fail)
  - [X] 10.7 Test login with confirmed email (should succeed)
  - [X] 10.8 Test profile update for admin and petugas
...
- [ ] 11.0 Documentation and deployment
  ...
  - [X] 11.3 Document email template configuration steps
  - [X] 11.4 Update migration guide for production deployment
  ...
  Notes:
  - PR created: https://github.com/dimyferdiana/zakat-fitrah-al-fajar/pull/2
  - 11.8 remains manual/dashboard step unless performed directly in Supabase Auth UI.
