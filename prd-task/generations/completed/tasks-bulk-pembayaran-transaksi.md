...
- [X] 7.0 Build, test, and deploy

  - [X] 7.1 Run `npm run build` — fix all TypeScript and lint errors.
  - [X] 7.2 Run `npm test` — confirm all new tests pass, 0 failures.
  - [X] 7.3 Manual QA: submit 5 muzakki with mixed types → verify 5 individual records in `pemasukan_uang`/`pemasukan_beras`.
  - [X] 7.4 Manual QA: verify receipt table layout matches reference photo.
  - [X] 7.5 Manual QA: create new muzakki inline → confirm DB record has only `nama` set.
  - [X] 7.6 Manual QA: confirm `hak_amil_snapshots` rows created for each inserted transaction.
