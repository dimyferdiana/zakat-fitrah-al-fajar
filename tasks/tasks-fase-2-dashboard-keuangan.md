...
- [ ] 1.0 Finalize data model & scope (Phase 2)
	- [ ] 1.1 Review PRD: scope in-scope vs out-of-scope (confirm no auto hak amil, akun fixed Kas/Bank)
	- [ ] 1.2 Decide schema approach: reuse `pembayaran_zakat` vs new tables for non-fitrah (recommended: new table)
	- [ ] 1.3 Define enums/values:
		- kategori pemasukan uang: `zakat_fitrah_uang`, `fidyah_uang`, `maal_penghasilan_uang`, `infak_sedekah_uang`
		- akun uang: `kas`, `bank`
	- [ ] 1.4 Define dashboard formulas:
...
- [ ] 2.0 Database migrations (pemasukan uang + hak amil manual + rekonsiliasi)
	...
	- [ ] 2.2 Create migration: `hak_amil` (manual per tahun)
		- [ ] 2.2.1 Columns (suggested): tahun_zakat_id (unique), jumlah_uang_rp, updated_by, updated_at
	- [ ] 2.3 Create migration: `rekonsiliasi` (admin-only)
		- [ ] 2.3.1 Columns (suggested): id, tahun_zakat_id, jenis (`uang`/`beras`), akun (nullable), jumlah_uang_rp (nullable), jumlah_beras_kg (nullable), tanggal, catatan, created_by, created_at
	- [ ] 2.4 RLS policies
		- [ ] 2.4.1 Petugas/admin can insert & read `pemasukan_uang`
		- [ ] 2.4.2 Only admin can insert/update `hak_amil` and `rekonsiliasi`
		- [ ] 2.4.3 Ensure selects for dashboard work with RLS (use views or policies as needed)
	- [ ] 2.5 Update generated types
		- [ ] 2.5.1 Update `zakat-fitrah-app/src/types/database.types.ts` to include new tables/types
	- [ ] 2.6 (Optional) Seed minimal records for active tahun (hak_amil default 0)

- [ ] 3.0 UI: Input pemasukan uang (Fidyah uang, Maal penghasilan uang, Infak/Sedekah)
	- [ ] 3.1 Decide UI placement
		- [ ] 3.1.1 Add a new menu page (recommended) vs reuse Muzakki page
	- [ ] 3.2 Build form: Pemasukan Uang (generic)
		- [ ] 3.2.1 Fields: kategori, akun (Kas/Bank), nominal, tanggal, catatan, (optional) muzakki
		- [ ] 3.2.2 Validation (zod): nominal > 0, tanggal required
	- [ ] 3.3 Implement specific entry flows
		- [ ] 3.3.1 Fidyah uang: create record kategori `fidyah_uang`
		- [ ] 3.3.2 Maal penghasilan uang: create record kategori `maal_penghasilan_uang`
		- [ ] 3.3.3 Infak/Sedekah uang: create record kategori `infak_sedekah_uang`
	- [ ] 3.4 Implement hooks (TanStack Query)
		- [ ] 3.4.1 `usePemasukanUangList` (filter by tahun, kategori, akun, date)
		- [ ] 3.4.2 `useCreatePemasukanUang` (mutation)
	- [ ] 3.5 Table/list view
...
- [ ] 4.0 UI: Dashboard update (Infak/Fidyah/Uang terkumpul + Hak Amil + rumus progress)
	- [ ] 4.1 Update `useDashboardStats` to include:
		- [ ] 4.1.1 Sum fidyah uang (from `pemasukan_uang`)
		- [ ] 4.1.2 Sum infak/sedekah uang (from `pemasukan_uang`)
		- [ ] 4.1.3 Sum maal penghasilan uang (from `pemasukan_uang`)
		- [ ] 4.1.4 Hak Amil uang (manual, from `hak_amil`)
		- [ ] 4.1.5 Total pemasukan uang (aggregate) and sisa uang formula
	- [ ] 4.2 Update Dashboard cards
		- [ ] 4.2.1 Add StatCard: Infak/Sedekah Uang Terkumpul
		- [ ] 4.2.2 Add StatCard: Fidyah Uang Terkumpul
		- [ ] 4.2.3 Decide where to show Maal (own card vs included in total)
	- [ ] 4.3 Update progress component
		- [ ] 4.3.1 Create new component `DistribusiProgressWithAmil` (recommended) OR extend existing component safely
